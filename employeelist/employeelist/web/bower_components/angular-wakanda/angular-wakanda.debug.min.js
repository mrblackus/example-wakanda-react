/*! angular-wakanda - v1.0.4 - 2016-02-23 */
(function(exports, global) {
    ...
    WAF.DataStore.funcCaller = function(methodref, from, params, options) {
        ...
        if ("entity" == methodref.applyTo) {
            ...
            if (null != entityCollection._private.dataURI) {
                var qsMethod;
                if (true === entityCollection._callWithEm) {
                    qsMethod = "&$emMethod=" + methodref.name;
                    method = "subentityset";
                    delete entityCollection._callWithEm;
                    if (entityCollection._private.dataURI.indexOf("?") === -1) qsMethod = "?" + qsMethod;
                } else qsMethod = "/" + methodref.name;
    ...
    WAF.EntityCollection.manageData = function(rawResult, init) {
        ...
        if (init) {
            ...
            if (/^\/rest\/\w+\/\$entityset\/[A-Z0-9]+(\?.*)?$/i.test(rawResult.__ENTITYSET)) priv.dataURI = rawResult.__ENTITYSET;
    ...
    wakanda.factory("$wakanda", [ "$q", "$rootScope", "$http", "$wakandaConfig", function($q, $rootScope, $http, $wakandaConfig) {
            ...
            wakandaUserDefinedMethodToPromisableMethods: function(method) {
                return function() {
                    ...
                    if ("$_entity" === mode) method.apply(this[mode], thatArguments); else {
                        ...
                        if (!this.$_collection && !(this instanceof WAF.EntityCollection)) throw new Error("Couldn't call user defined method on collection because no pointer on this collection");
        ...
        var transform = {
            ...
            addUserDefinedMethodsToCollection: function(result, root) {
                ...
                } else if (false === root && null !== result.$_collection.relEntityCollection) if ("undefined" !== typeof result.$_collection && "undefined" !== result.$_collection.relEntityCollection) {
                    dataClass = result.$_collection.relEntityCollection;
                    dataClass._private.entityCollectionMethods = {};
                    for (var m in dataClass._private.methods) if (dataClass._private.methods.hasOwnProperty(m)) dataClass._private.entityCollectionMethods[m] = dataClass[m];
                }
                ...
                for (var methodName in userDefinedEntityCollectionMethods) if (userDefinedEntityCollectionMethods.hasOwnProperty(methodName)) if (true === root) result[methodName] = userDefinedEntityCollectionMethods[methodName]; else result[methodName] = userDefinedEntityCollectionMethods[methodName].bind(result.$_collection.relEntityCollection);
                return result;
            },
                ...
                dataClass.getAttributes().forEach(function(attr) {
                    if ("relatedEntity" === attr.kind) Object.defineProperty(this, attr.name, {
                        ...
                        get: function() {
                            ...
                            if (this.$_entity[attr.name].relEntityCollection && this.$_entity[attr.name].value.__ENTITYSET) this.$_entity[attr.name].relEntityCollection._private.dataURI = this.$_entity[attr.name].value.__ENTITYSET;
